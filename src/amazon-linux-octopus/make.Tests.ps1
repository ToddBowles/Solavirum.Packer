...
Describe $ami -Tags @("Ignore") {
    Context "When running the make" {
        It "No errors are thrown and an AMI is created" {
            ...
            $octoCreds = @{ 
                Key = (Get-CredentialByKey "OCTOPUS_TESTS_OCTOPUS_API_KEY"); 
                Url = (Get-CredentialByKey "OCTOPUS_TESTS_OCTOPUS_API_URL"); 
            };
            $makeArgs = @{
                AwsKey=$creds.AwsKey;
                ...
                ParameterOverrides=@{
                    octopus_url=$octoCreds.Url;
                    octopus_key=$octoCreds.Key;
                };
            }
